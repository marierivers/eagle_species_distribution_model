...
This analysis uses machine learning techniques to model the distribution of bald eagles (Haliaeetus leucocephalus) based on species observations from the Global Biodiversity Information Facility (GBIF.org) and environmental data.
...
Maxent is a commonly used species distribution model that performs well with few input data points and only requires presence points. Since this model only has presence points, it samples the background for comparison.
```{r warning=FALSE}
# load extra packages
librarian::shelf(
  maptools, sf)
...
```
The true positive rate = `r tpr`
The true negative rate = `r tnr`
The false positive rate = `r fpr`
The false negative rate = `r fnr`
